...
def pred_loss_func(model, x, y, mask, key, inference=False):
    logits = model(x, key, inference)
...
def masked_kl_div(opt_dist, mdl_dist, mask):
    ...
    token_kl_div = jnp.sum(kl_div(opt_dist, mdl_dist), -1)
    return jnp.sum(token_kl_div * mask) / jnp.sum(mask)
